...
public class StickerView extends FrameLayout {
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            int borderPadding = a.getInteger(R.styleable.StickerView_borderPadding, 0);
            bringToFrontCurrentSticker =
                    a.getBoolean(R.styleable.StickerView_bringToFrontCurrentSticker, false);

            borderPaint.setAntiAlias(true);
            borderPaint.setColor(a.getColor(R.styleable.StickerView_borderColor, Color.TRANSPARENT));
//            borderPaint.setAlpha(a.getInteger(R.styleable.StickerView_borderAlpha, 128));
    ...
    public void configDefaultIcons() {
        BitmapStickerIcon deleteIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.mipmap.icon_sticker_delete),
                BitmapStickerIcon.LEFT_TOP);
        deleteIcon.setIconEvent(new DeleteIconEvent());
        BitmapStickerIcon zoomIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.mipmap.icon_sticker_drag),
                BitmapStickerIcon.RIGHT_BOTOM);
        zoomIcon.setIconEvent(new ZoomIconEvent());
        BitmapStickerIcon flipIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.mipmap.icon_sticker_flip),
                ...
                for (int i = 0; i < icons.size(); i++) {
                    ...
                    switch (icon.getPosition()) {
                        ...
                        case BitmapStickerIcon.RIGHT_TOP:
                            if (handlingSticker instanceof TextSticker || handlingSticker instanceof BubbleSticker || handlingSticker instanceof DownTimerSticker) {
                        ...
                        case BitmapStickerIcon.LEFT_BOTTOM:
                            if (handlingSticker instanceof TextSticker || handlingSticker instanceof BubbleSticker) {
                        ...
                        case BitmapStickerIcon.RIGHT_BOTOM:
                            if (!(handlingSticker instanceof DownTimerSticker)) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (findCurrentIconTouched() != null || findHandlingSticker() != null) {
                    ...
                    if (onStickerOperationListener != null) {
                        onStickerOperationListener.onClickStickerOut();
                    }
                    return false;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() > 1 && handlingSticker instanceof DownTimerSticker) {
            return super.onTouchEvent(event);
        }
    ...
    public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null && !(sticker instanceof DownTimerSticker)) {
            ...
            float scaleValue = newDistance / oldDistance;
            if (Math.min(handlingSticker.getCurrentWidth(), handlingSticker.getCurrentHeight()) < 55f
                    && scaleValue < 1f) {
                return;
            }
            moveMatrix.set(downMatrix);
            moveMatrix.postScale(scaleValue, scaleValue, midPoint.x, midPoint.y);
            moveMatrix.postRotate(newRotation - oldRotation, midPoint.x, midPoint.y);
            handlingSticker.setMatrix(moveMatrix);
//            Log.d("StickerView", "handlingSticker = " + handlingSticker.getCurrentWidth());
        }
    }
    ...
    public void copySticker(@Nullable Sticker sticker) {
        ...
        if (sticker instanceof DrawableSticker) {
            newSticker = new DrawableSticker(sticker.getDrawable());
        } else if (sticker instanceof HeadSticker) {
            newSticker = new HeadSticker(sticker.getDrawable());
        } else if (sticker instanceof BubbleSticker) {
            newSticker = new BubbleSticker(sticker.getDrawable(), ((BubbleSticker) sticker).getBubbleTemplateInfo());
        } else if (sticker instanceof TimeSticker) {
            newSticker = new TimeSticker(sticker.getDrawable());
        } else if (sticker instanceof StateSticker) {
            newSticker = new StateSticker(sticker.getDrawable());
        } else if (sticker instanceof DownTimerSticker) {
            newSticker = new DownTimerSticker(sticker.getDrawable(), ((DownTimerSticker) sticker).getDowntimerEntity());
        }
    ...
    public void setStickerCenter(Sticker sticker) {
        if (sticker != null) {
            setStickerCenterHorizontal();
            setStickerCenterVertical();
        }
    }
     ...
     * 水平居中
     */
    public void setStickerCenterHorizontal() {
        if (handlingSticker != null) {
            moveMatrix.set(handlingSticker.getMatrix());
            float values[] = new float[9];
            moveMatrix.getValues(values);
            RectF rectF = mathRealSize();
            //边框距离还有10dp
            values[Matrix.MTRANS_X] = getWidth() / 2f - rectF.width() / 2f + dip2px(getContext(), 10);
            moveMatrix.setValues(values);
            handlingSticker.setMatrix(moveMatrix);
            invalidate();
        }
    }
     ...
     * 竖直居中
     */
    public void setStickerCenterVertical() {
        if (handlingSticker != null) {
            moveMatrix.set(handlingSticker.getMatrix());
            float values[] = new float[9];
            moveMatrix.getValues(values);
            RectF rectF = mathRealSize();
            values[Matrix.MTRANS_Y] = getHeight() / 2f - rectF.height() / 2f;
            moveMatrix.setValues(values);
            handlingSticker.setMatrix(moveMatrix);
            invalidate();
        }
    ...
    private RectF mathRealSize() {
        ...
        for (int i = 0; i < 8; i++) {
            if (i % 2 == 0) {
                xList.add(boundPoints[i]);
            } else {
                yList.add(boundPoints[i]);
            }
        }
        //横坐标排序
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3 - i; j++) {
                if (xList.get(j) < xList.get(j + 1)) {
                    float temp = xList.get(j);
                    xList.set(j, xList.get(j + 1));
                    xList.set(j + 1, temp);
                }
            }
        }
        rectF.left = xList.get(3);
        rectF.right = xList.get(0);

        //纵坐标排序
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3 - i; j++) {
                if (yList.get(j) < yList.get(j + 1)) {
                    float temp = yList.get(j);
                    yList.set(j, yList.get(j + 1));
                    yList.set(j + 1, temp);
                }
            }
        }
        rectF.top = yList.get(3);
        rectF.bottom = yList.get(0);
        return rectF;
    }
    ...
    public boolean removeCurrentSticker() {
        return handlingSticker != null && remove(handlingSticker);
    }
    ...
    /**
     * 直接传入Matrix来添加
     * 不要怕就是用自己写的方法不知道有没有问题
     *
     * @param sticker   要添加的贴纸
     * @param oldMatrix matrix
     */
    public void addSticker(final @NonNull Sticker sticker, final Matrix oldMatrix) {
        if (ViewCompat.isLaidOut(this)) {
            addStickerImmediately(sticker, oldMatrix);
        } else {
            post(new Runnable() {
                @Override
                public void run() {
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position) {
        //如果是图片贴纸的话 就让它的大小为 宽度或者高度的1/3
        if (sticker instanceof HeadSticker && sticker.getDrawable().getIntrinsicHeight() != 0) {
            float size = dip2px(getContext(), 45);
            float scaleFactor = size / (float) sticker.getDrawable().getIntrinsicWidth();
            sticker.getMatrix().postScale(scaleFactor, scaleFactor, sticker.getDrawable().getIntrinsicWidth() / 2, sticker.getDrawable().getIntrinsicWidth() / 2);
        } else if (sticker instanceof DrawableSticker && sticker.getDrawable().getIntrinsicHeight() != 0) {
            ...
            sticker.getMatrix().postScale(scaleFactor / 3, scaleFactor / 3, getWidth() / 3, getHeight() / 3);
        } else if ((sticker instanceof BubbleSticker || sticker instanceof DownTimerSticker) && sticker.getDrawable().getIntrinsicHeight() != 0) {
            //气泡图处理
            if (sticker.getDrawable().getIntrinsicWidth() > getWidth() || sticker.getDrawable().getIntrinsicHeight() > getHeight()) {
                //只要 宽高其中一个大于stickerView的宽高 就表示需要进行缩放
                float scaleFactor, widthScaleFactor, heightScaleFactor;
                widthScaleFactor = (float) getWidth() / sticker.getDrawable().getIntrinsicWidth();
                heightScaleFactor = (float) getHeight() / sticker.getDrawable().getIntrinsicHeight() * 0.8f;
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        if ((position & Sticker.Position.TOP) > 0) {
            ...
            offsetY /= (sticker instanceof DrawableSticker ? 2.5f : 2f);
        }
        if ((position & Sticker.Position.LEFT) > 0) {
            ...
            offsetX /= (sticker instanceof DrawableSticker ? 2.5f : 2f);
        }
        if (!(sticker instanceof DrawableSticker) && !(sticker instanceof HeadSticker)) {
            sticker.getMatrix().reset();
        }
        sticker.getMatrix().postTranslate(offsetX, offsetY);
    }
     ...
     * 重置角度
     *
     * @param newAngle 要旋转的新角度
     */
    public void resetAngle(int newAngle) {
        if (handlingSticker != null) {
            moveMatrix.set(handlingSticker.getMatrix());
            float angle = handlingSticker.getMatrixAngle(moveMatrix);
            PointF centerPointF = handlingSticker.getMappedCenterPoint();
            moveMatrix.postRotate(newAngle - angle, centerPointF.x, centerPointF.y);
            handlingSticker.setMatrix(moveMatrix);
            invalidate();
        }
    }
    ...
    public Bitmap createBitmap() throws OutOfMemoryError {
        handlingSticker = null;
        float scale = ((float)ExtKt.getScreenSizeWidth()) / getWidth();
    ...
    public interface OnStickerOperationListener {
        ...
        void onClickStickerOut();
    }

    public static class CountListener implements OnStickerOperationListener{
